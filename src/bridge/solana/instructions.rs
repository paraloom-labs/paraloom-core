...
pub mod discriminators {
    ...
    /// Run this before `reset_validator_registry` when excluding an active PDA,
    /// so the reset can rebuild the final aggregate counters last.
...
/// and drops its stake from the registry counters. During reconciliation, call
/// this before `reset_validator_registry`, which should rebuild the final
/// aggregate counters last. Does not move the staked lamports.
