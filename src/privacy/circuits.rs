...
//!
//! Circuit types:
//! - TransactCircuitV3: unified shielded transact (deposit/transfer/withdraw)
//! - {Deposit,Transfer,Withdraw}CircuitV2: spend-key deposit/transfer/withdraw
...
///
/// Spend authority is a private key, not a free `secret` witness. The note binds
/// `pubkey = Poseidon(privkey)` in its commitment, and the nullifier folds in a
/// signature over `(commitment, leaf_index)` that requires the private key — so a
/// note at a given tree position yields exactly one nullifier and only its
/// key-holder can produce it. `leaf_index` is derived in-circuit from the Merkle
/// path's direction bits, so the prover cannot pick an index inconsistent with
/// the path. This is the live withdraw circuit; the v1 predecessor has been
/// removed.
...
///
/// The shielded→shielded spend-key transfer circuit. Each input note binds
...
/// presented as two inputs. This is the live transfer circuit; the v1
...
///
/// The public→shielded spend-key deposit circuit: it proves the output note's
/// commitment is a well-formed spend-key commitment binding the recipient's spend
/// public key. A deposit creates a note rather than spending one, so there is no
/// nullifier, Merkle path or signature — the depositor only needs the recipient's
/// *public* key (only the recipient, holding the matching private key, can later
/// spend it). This is the live deposit circuit; the v1 predecessor has been
/// removed.
